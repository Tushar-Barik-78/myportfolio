...
const Works = () => {
  ...
  return (
    ...
    >
      ...
      {/* MODAL (AnimatePresence - Responsive & Scrollable Version) */}
      <AnimatePresence>
        {selectedProject && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            // overlay me overflow-y-auto aur padding lagayi hai mobile scroll handle karne ke liye
            className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md p-3 sm:p-4 overflow-y-auto generic-scrollbar"
          >
            <motion.div
              ref={workBox}
              initial={{ scale: 0.9, opacity: 0, y: 20 }}
              ...
              // max-height restrict karke elastic box implementation kiya hai
              className="w-full max-w-2xl my-auto rounded-2xl md:rounded-3xl overflow-hidden border border-blue-900/40 bg-[#090d1a] shadow-2xl relative flex flex-col max-h-[92vh]"
            >
              {/* CLOSE BUTTON */}
              <button
                onClick={() => setSelectedProject(null)}
                className="absolute top-3 right-3 z-20 w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-black/60 border border-blue-900/50 text-white flex items-center justify-center hover:bg-red-500/20 hover:border-red-500/40 transition-all duration-200"
              >
                <FiX size={16} />
              </button>

              {/* MODAL BODY SCROLLABLE WRAPPER */}
              <div className="overflow-y-auto flex-1 flex flex-col popup-inner-scroll">
                {/* MODAL IMAGE */}
                <div className="relative w-full aspect-video bg-blue-950/20 shrink-0">
                  {selectedProject.image ? (
                    <img
                      src={selectedProject.image}
                      alt={selectedProject.title}
                ...
                <div className="p-5 sm:p-7 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-xs sm:text-sm md:text-base text-gray-300 mb-4 sm:mb-5 leading-relaxed">
                      {selectedProject.description}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-6">
                      {(selectedProject.tags || []).map((tag, i) => (
                        <span
                          key={i}
                          className="bg-blue-950/50 border border-blue-900/40 text-[11px] sm:text-xs font-medium text-cyan-400 rounded-md px-2.5 py-1"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  {/* ACTION BUTTONS (Sticky/Flexible at bottom) */}
                  <div className="flex flex-row gap-3 mt-auto pt-2 border-t border-blue-900/20">
                    {selectedProject.github && (
                      <a
                        href={selectedProject.github}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-blue-950/40 border border-blue-900/40 text-white text-xs sm:text-sm font-semibold hover:bg-blue-900/30 transition-all duration-200"
                      >
                        <FiGithub size={16} />
                        Code
                      </a>
                    ...
                    {selectedProject.webapp && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white text-xs sm:text-sm font-semibold shadow-lg shadow-blue-500/10 transition-all duration-200"
                      >
